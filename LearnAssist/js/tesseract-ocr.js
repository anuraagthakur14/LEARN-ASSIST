		...
		.then(function (data) {
			...
			axios.post('http://localhost:5500/api', {
				...
				.then(function (response) {
					...
					doc.innerHTML = "<br/> Key Phrases/Terms: <br/><br/>";

					for (var i = 0; i < response.data.length; i++) {
						var btn = document.createElement("a");
						btn.className = "btn btn-primary";
						btn.style.margin = 0.80;
						btn.href = "https://www.google.com/search?q=" + response.data[i];
						btn.target = "_blank";
						btn.innerHTML = response.data[i];
						doc.appendChild(btn);
					}
				})
...
var inputForm = document.querySelector('#tts');
var inputTxt = document.querySelector('#text-to-speech');
...
function textSelection(event) {
	const text = document.getElementById('text-to-speech');
	const text2 = document.getElementById('translation');
	const selected = document.getSelection().toString();
	text.value = selected;
	text2.value = selected;
}
